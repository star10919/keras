...
ic(y_samsung.shape)     #  y_samsung.shape: (2596,)
...
# lstm = LSTM(10, activation='relu', return_sequences=True)(input1)
# conv = Conv1D(64, 1, activation='relu')(lstm)
# flat = Flatten()(conv)
...
# lstm = LSTM(10, activation='relu', return_sequences=True)(input2)
# conv = Conv1D(64, 1, activation='relu')(lstm)
# flat = Flatten()(conv)
# dense = Dense(64, activation='relu')(flat)
# dense = Dense(10, activation='relu')(dense)
...
model = load_model('./_save/pred_stock_model_save1.h5')
...
#             save_best_only=True, 
#             filepath= './_save/ModelCheckPoint/pred_stock_MCP1.py')

# model.fit([sam_x_train, sk_x_train], sam_y_train, epochs=50, batch_size=50, validation_split=0.02, callbacks=[es, cp])


# model.save('./_save/pred_stock_model_save1.h5')
# model.save_weights('./_save/pred_stock_weight_save1.h5')
