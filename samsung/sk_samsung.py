...
x_samsung_pred = x_samsung[-1, :]
x_sk_pred = x_sk[-1, :]

ic(x_samsung.shape)     #  x_samsung.shape: (2597, 5, 5)
ic(x_sk.shape)          #  x_sk.shape: (2597, 5, 5)
ic(x_samsung_pred.shape)    # x_samsung_pred.shape: (5, 5)

y_samsung = samsung[4:, 4]    # 삼성종가
ic(y_samsung.shape)     #  y_samsung.shape: (2597,)

x_samsung = x_samsung.reshape(2597,25)
x_sk = x_sk.reshape(2597,25)
...
'''
    sam_x_train.shape: (2077, 5, 5)
    sam_x_test.shape: (520, 5, 5)
    x_samsung_pred.shape: (5, 5)
    sk_x_train.shape: (2077, 5, 5)
...
from tensorflow.keras.layers import Dense, Conv1D, Conv2D, MaxPool2D, LSTM, Flatten, GlobalAveragePooling2D, Input, Dropout

# 모델1 - samsung
input1 = Input(shape=(5, 5))
lstm = LSTM(100, activation='relu', return_sequences=True)(input1)
...
lstm = LSTM(100, activation='relu', return_sequences=True)(input2)
conv = Conv1D(64, 2, activation='relu')(lstm)
flat = Flatten()(conv)
dense = Dense(64, activation='relu')(flat)
dense = Dense(64, activation='relu')(dense)
...
cp = ModelCheckpoint(monitor='val_loss', mode='auto', verbose=1,
            save_best_only=True, 
            filepath= './_save/ModelCheckPoint/pred_stock_MCP11.py')

model.fit([sam_x_train, sk_x_train], sam_y_train, epochs=50, batch_size=50, validation_split=0.02, callbacks=[es, cp])


model.save('./_save/pred_stock_model_save11.h5')
model.save_weights('./_save/pred_stock_weignt_save11.h5')
...
Epoch 00027: val_loss did not improve from 1375029.00000
Epoch 00027: early stopping
17/17 [==============================] - 0s 9ms/step - loss: 15713079.0000
loss : 15713079.0
ic| y_predict: array([[81091.5]], dtype=float32)


*'./_save/pred_stock_model_save11.h5'
Epoch 00030: val_loss did not improve from 1428108.37500
Epoch 00030: early stopping
17/17 [==============================] - 0s 8ms/step - loss: 6147139.5000
loss : 6147139.5
ic| y_predict: array([[81171.21]], dtype=float32)
'''